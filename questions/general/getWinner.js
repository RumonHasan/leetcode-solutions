const getWinner = (arr, k) => {
  ...
  while (consequtive_wins < k) {
    if (arr[0] > arr[1]) {
      ...
      consequtive_wins = 1;
      let winner = arr[1]; // storing winner then splicing it as it has to be moved
      arr.splice(1, 1);
      arr.push(arr[0]);
      arr[0] = winner;
    }
...
// getting the longest common sub
const longestCommonSub = (text1, text2) => {
  let mat = Array(text1.length + 1)
    .fill()
