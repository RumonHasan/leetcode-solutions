const numDifferentIntegers = (word) => {
  // git add
  let integerArray = [];
  let numString = '';
  let infinityCounter = 0;
  for (let index in word) {
    if (!isNaN(Number(word[index]))) {
      numString += Number(word[index]);
    } else {
      if (numString !== '') {
        if (Number(numString) === Infinity) {
          infinityCounter++;
        }
        integerArray.push(Number(numString));
      }
      numString = '';
    }
    if (Number(index) === word.length - 1 && numString.length > 0) {
      if (Number(numString) === Infinity) {
        infinityCounter++;
      }
      integerArray.push(Number(numString));
    }
  }
  let newIntegerArray = integerArray.filter((number) => number !== Infinity);
  const set = new Set([...newIntegerArray]);
  return set.size + infinityCounter;
};

// console.log(
//   numDifferentIntegers(
//     ''
//   )
// );
